...
import type { Goal, ScanFeatures } from '../engine/types'
...
function observations(
  ...
  if (goals.includes('calm')) out.push({ icon: '🧘', key: 'fbStress' })
...
export function Feedback({ lang, active, variant, scan, values, goals, onDone }: Props) {
  useEffect(() => {
    if (!active) return
  ...
  const obs = observations(scan, values, goals)

  return (
    <section className={`kiosk-page k-center k-feedback ${active ? 'active' : ''}`} onClick={onDone}>
      <div className="k-fb-seal">{SEALS[variant] ?? SEALS[0]}</div>
      <h2 className="k-title">{t(lang, TITLES[variant] ?? TITLES[0])}</h2>
